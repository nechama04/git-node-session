...
const Register = () => {
    ...
    const handleRegister = () => {
        ...
        const data = { userName: userName, password: password, name: name, email: email, phone: phone, address: address }
        console.log("ddd",data);
        setVisible(false)
        while(!userName || !password)
            alert("userName and password is required")
            while(userName.toLowerCase() != userName) alert("userName must be lowercase!")
    ...
    return (
        <div className="card flex justify-content-center">
            {/* <Button label="Register" icon="pi pi-user" onClick={() => setVisible(true)} /> */}
            <Dialog
                visible={visible}
                modal
                ...
                content={({ hide }) => (
                    <div className="flex flex-column px-8 py-5 gap-4" style={{ borderRadius: '12px', backgroundImage: 'radial-gradient(circle at left top, var(--primary-400), var(--primary-700))' }}>
                        ...
                        <div className="inline-flex flex-column gap-2">
                            ...
                            <InputText id="name" label="Name" className="bg-white-alpha-20 border-none p-3 text-primary-50" onChange={(e) => setName(e.target.value)}></InputText>
                        </div>
                        <div className="inline-flex flex-column gap-2">
                            <label htmlFor="password" className="text-primary-50 font-semibold">
                                Email
                        ...
                        <div className="inline-flex flex-column gap-2">
                            ...
                            <InputText id="phone" label="Phone" className="bg-white-alpha-20 border-none p-3 text-primary-50" onChange={(e) => setAddress(e.target.value)}></InputText>
                        </div>
                        <div className="inline-flex flex-column gap-2">
                            <label htmlFor="password" className="text-primary-50 font-semibold">
                                Address
                            </label>
                            <InputText id="address" label="Address" className="bg-white-alpha-20 border-none p-3 text-primary-50" onChange={(e) => handleChange(e)}></InputText>
                        </div>
                        <div className="flex align-items-center gap-2">
                            <Button label="Register" onClick={handleRegister} text className="p-3 w-full text-primary-50 border-1 border-white-alpha-30 hover:bg-white-alpha-10"></Button>
